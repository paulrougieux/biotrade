...
Script used to compute for each exporting country the relative and absolute change value on deforestation products traded to EU and ROW.
...
# Compute the relative and absolute change in time and return the last value related to the average of 5 previous years
trade_change = relative_absolute_change(trade_data_agg, years=5, last_value=True)
# Save the file in the biotrade script folder (ignored by git)
faostat_commodity_change_file = (
    Path.cwd() / "scripts" / "faostat_commodity_change_export.csv"
)
trade_change.to_csv(
    faostat_commodity_change_file, index=False, encoding="latin1", na_rep="NA",
)
